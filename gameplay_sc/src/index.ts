...
import { checkWalletBalances } from './contractHelpers';
import type { Nation, GameState } from './types';
...
function showGameDecisions(game: GameState) {
...
async function processGame(gameId: string) {
    ...
    const isReady = await isGameReady(gameId);
    if (!isReady) {
    ...
    try {
        ...
        await updateGameStatus(gameId, 'ready_for_processing_alliance');
...
async function processAllianceDecisions(gameId: string) {
    ...
    try {
        ...
        await updateGameStatus(gameId, 'completed_alliance_processing');
    } catch (error) {
...
async function processWinner(gameId: string) {
    ...
    const result = await calculateWinner(gameId);
    if (!result) {
...
async function startGame() {
    try {
        ...
        console.error("Please check your .env configuration and network connection.");
        return;
    }
    ...
    while (true) {
        ...
        console.log("8. Check Wallet Balances");
        console.log("9. Exit");
        ...
        try {
            if (choice === "1") {
                ...
                if (player1Nation) {
                    ...
                    await updateGameStatus(gameId, 'player1_completed');
